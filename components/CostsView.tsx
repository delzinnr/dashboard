    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-6">
        <div className="flex items-end justify-between">
          ...
          <button onClick={() => setIsModalOpen(true)} className="lg:hidden w-12 h-12 bg-red-500 text-white rounded-xl shadow-lg shadow-red-500/20 flex items-center justify-center">
      ...
      <div className="lg:hidden space-y-3">
        {costs.length > 0 ? costs.map(cost => (
          <div key={cost.id} className="bg-[#0c0c0c] border border-white/5 p-4 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-zinc-400">{getIcon(cost.type)}</div>
              <div>
            ...
            <div className="flex flex-col items-end">
              ...
              <button onClick={() => onDeleteCost(cost.id)} className="text-[9px] font-black text-zinc-800 uppercase mt-1">Excluir</button>
            </div>
          </div>
      ...
      <div className="hidden lg:block bg-[#0c0c0c] rounded-[2rem] overflow-hidden border border-white/5">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-white/5">
            {costs.map(cost => (
              <tr key={cost.id} className="hover:bg-white/[0.02]">
                <td className="px-8 py-6">
                  <p className="font-bold text-white text-sm">{cost.name}</p>
                  <p className="text-[10px] text-zinc-600 font-bold">{cost.date}</p>
                </td>
                <td className="px-8 py-6">
                  <span className="text-[10px] font-black text-zinc-500 uppercase flex items-center gap-2">{getIcon(cost.type)} {cost.type}</span>
                </td>
                ...
                <td className="px-8 py-6 text-right">
                  <button onClick={() => onDeleteCost(cost.id)} className="p-2 text-zinc-700 hover:text-red-500"><Trash2 size={18} /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-4 bg-black/95 backdrop-blur-md">
          <div className="bg-[#0a0a0a] w-full h-full md:h-auto md:max-w-md md:rounded-[2rem] border-white/10 p-8 flex flex-col">
            ...
            <form onSubmit={e => { e.preventDefault(); onAddCost(newCost); setIsModalOpen(false); }} className="space-y-6 flex-1">
              <div className="space-y-2"><label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Descrição</label><input type="text" className="w-full bg-black border border-white/5 rounded-2xl py-4 px-6 text-white outline-none focus:border-red-500/30" value={newCost.name} onChange={e => setNewCost({...newCost, name: e.target.value})} required placeholder="Ex: Proxy Semanal" /></div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2"><label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Categoria</label><select className="w-full bg-black border border-white/5 rounded-2xl py-4 px-6 text-xs text-white outline-none" value={newCost.type} onChange={e => setNewCost({...newCost, type: e.target.value as CostType})}><option value="sms">SMS</option><option value="proxy">Proxy</option><option value="ferramenta">Ferramenta</option><option value="outros">Outros</option></select></div>
                <div className="space-y-2"><label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Valor (R$)</label><input type="number" step="0.01" className="w-full bg-black border border-white/5 rounded-2xl py-4 px-6 text-white outline-none" value={newCost.amount} onChange={e => setNewCost({...newCost, amount: Number(e.target.value)})} required /></div>
              </div>
              <div className="flex gap-4 pt-10 mt-auto md:mt-4">
                ...
                <button type="submit" className="flex-1 bg-white text-black py-4 rounded-2xl text-[11px] font-black uppercase shadow-xl hover:bg-red-500 hover:text-white transition-all">Lançar Agora</button>
              </div>
            </form>
          </div>
        </div>
