...
export const CyclesView: React.FC<CyclesViewProps> = ({ cycles, onAddCycle, onDeleteCycle, onDeleteMultipleCycles, userRole }) => {
  ...
  const filteredCycles = useMemo(() => {
    ...
    return cycles.filter(c => {
      ...
      let matchManual = true;
      if (showFilters) {
        const cycleTime = cycleDate.getTime();
        const matchStart = !startDate || cycleTime >= new Date(startDate).getTime();
        const matchEnd = !endDate || cycleTime <= new Date(endDate).getTime();
        matchManual = matchStart && matchEnd;
      }
      return matchSearch && (showFilters ? matchManual : matchTimeframe);
    });
  }, [cycles, searchTerm, timeframe, timeframeInfo, startDate, endDate, showFilters]);
  ...
  return (
    <div className="space-y-6 relative min-h-screen">
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-1">Registros Operacionais</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="flex bg-[#0c0c0c] p-1 rounded-2xl border border-white/5 gap-0.5">
            ...
            <button onClick={() => { setEditingCycle(null); setIsModalOpen(true); }} className="flex items-center justify-center w-12 h-12 bg-yellow-500 text-black rounded-xl shadow-lg hover:bg-yellow-400 transition-all">
      ...
      <div className="bg-[#0c0c0c] rounded-[2.5rem] overflow-hidden border border-white/5 shadow-2xl">
        <table className="w-full text-left">
          <thead className="bg-black/40 text-zinc-600 text-[10px] font-black uppercase tracking-widest">
            <tr>
              ...
              <th className="px-8 py-8">Lucro Bruto</th>
          ...
          <tbody className="divide-y divide-white/5">
            {filteredCycles.map(cycle => (
              <tr 
                key={cycle.id} 
                className={`hover:bg-white/[0.01] transition-all duration-300 ${deletingId === cycle.id ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}
              >
                ...
                <td className="px-8 py-8 text-right">
                  <div className="flex items-center justify-end gap-3">
                    <button onClick={() => { setEditingCycle(cycle); setIsModalOpen(true); }} className="p-3 bg-white/5 text-zinc-400 hover:text-blue-500 rounded-2xl transition-all"><Edit3 size={18} /></button>
                    <button onClick={() => handleDelete(cycle.id)} className="p-3 bg-white/5 text-zinc-400 hover:text-red-500 rounded-2xl transition-all"><Trash2 size={18} /></button>
                  </div>
                </td>
              </tr>
            ))}
      ...
      {showScrollTop && (
        <button 
          onClick={scrollToTop}
          className="fixed bottom-24 lg:bottom-10 right-4 lg:right-10 w-12 h-12 bg-yellow-500 text-black rounded-full flex items-center justify-center shadow-2xl z-[90] animate-in zoom-in duration-300 hover:scale-110 active:scale-95"
        >
